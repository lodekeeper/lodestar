...
function computePostState(
  ...
  return stateTransition(parentState, signedBlock, {
...
export async function runGossipValidationTest(
  ...
  const cachedState = createCachedBeaconState(
    anchorState,
    {config: beaconConfig, pubkey2index, index2pubkey},
  ...
  try {
    ...
    const blockStatesByRoot = new Map<RootHex, CachedBeaconStateAllForks>();
    ...
    if (meta.blocks) {
      for (const [index, blockEntry] of meta.blocks.entries()) {
        ...
        if (index === 0) {
          blockStatesByRoot.set(blockRootHex, cachedState);
          continue;
        }
...
async function validateMessageForTopic(
  ...
  switch (topic) {
    ...
    case GossipType.bls_to_execution_change: {
      ...
      const change = rejectOnInvalidSerializedBytes(() => ssz.capella.SignedBLSToExecutionChange.deserialize(bytes));
