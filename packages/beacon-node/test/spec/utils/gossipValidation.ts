...
function loadTestCaseChainConfig(testCaseDir: string, fork: ForkName) {
  ...
  const configJson = {} as Record<string, string>;
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (!(key in chainConfigTypes)) continue;
...
function computePostState(
  ...
  signedBlock: ReturnType<(typeof sszTypesFor<ForkName>)["SignedBeaconBlock"]["deserialize"]>,
  fork: ForkName
): IBeaconStateView {
...
async function validateMessageForTopic(
  ...
  switch (topic) {
    ...
    case GossipType.bls_to_execution_change: {
      const capellaStartSlot = computeStartSlotAtEpoch(chain.config.CAPELLA_FORK_EPOCH);
      if (chain.clock.currentSlot < capellaStartSlot) {
